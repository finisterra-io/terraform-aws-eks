...
| <a name="input_cluster_encryption_config"></a> [cluster\_encryption\_config](#input\_cluster\_encryption\_config) | Configuration block with encryption configuration for the cluster. To disable secret encryption, set this value to `{}` | <pre>object({<br>    provider = list(object({<br>      key_arn = string<br>    }))<br>    resources = list(string)<br>  })</pre> | `null` | no |
